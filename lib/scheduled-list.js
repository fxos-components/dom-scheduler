(function(exports) {
  ...
  list.prototype = {
    ...
    _reorderStart: function(evt) {
      ...
      ctx.item = li;
      ctx.initialY = evt.touches[0].pageY;
      ctx.moveUp = new Set();
      ...
      toggleIndexPromotion(this.scheduler, li)
        .then(listenToMove)
        .then(toggleDraggingClass.bind(null, this.scheduler, li));
    },
    ...
    _reorderEnd: function(evt) {
      ...
      var position = evt.changedTouches[0].pageY;
      var delta = position - ctx.initialY;
      computeChanges(ctx, this.geometry, this._itemsInDOM, delta);

      Promise.all([
        applyChanges(this.scheduler, ctx, this.geometry, this._itemsInDOM),
        moveInPlace(this.scheduler, ctx, this.geometry)
      ]).then(this._commitToDocument.bind(this))
        .then(toggleIndexPromotion.bind(null, this.scheduler, li))
        .then(toggleDraggingClass.bind(null, this.scheduler, li))
        .then(function() {
          ctx.item = null;
          ctx.initialY = null;
  ...
  function toggleIndexPromotion(scheduler, item) {
    return scheduler.mutation(function() {
      item.style.zIndex = item.style.zIndex ? '' : 1000;
    });
  }

  function toggleDraggingClass(scheduler, item) {
    return scheduler.transition(function() {
  ...
  function applyChanges(scheduler, context, geometry, domItems) {
    ...
    for (var i = 0; i < domItems.length; i++) {
      ...
      if (context.moveDown.has(item)) {
        if (item.dataset.tweakDelta !== itemHeight) {
      ...
      if (context.moveUp.has(item)) {
        if (item.dataset.tweakDelta !== itemHeight * -1) {
