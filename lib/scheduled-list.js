(function(exports) {
  ...
  function recycle(items, start, end) {
    var recyclableItems = [];
    for (var i in items) {
      if ((i < start) || (i > end)) {
  ...
  list.prototype = {
    ...
    _reorderMove: function(evt) {
      ...
      this.scheduler.direct((function() {
        ...
        this._quietTimeout = setTimeout(this._rearrange.bind(this, li, delta), 160);
    ...
    _pushDown: function() {
      ...
      }, itemsInDOM[0], 'transitionend', 250);
    },
    ...
    _reveal: function() {
      ...
      return this.scheduler.transition(function() {
        newEl.style.transition = 'opacity 0.15s ease-out';
        newEl.style.webkitTransition = 'opacity 0.15s ease-out';
        newEl.classList.remove('new');
      }, newEl, 'transitionend', 200).then(function() {
    ...
    _updateRenderingWindows: function() {
      var itemPerScreen = this._viewportHeight / this._itemHeight;
      // Taking into account the will-change budget
      this._maxItemCount = Math.floor(itemPerScreen * 3);
      this._switchWindow = Math.floor(itemPerScreen / 2);

      log('maxItemCount: ' + this._maxItemCount);
    },
    ...
    handleScroll: function(evt) {
      ...
      this.scheduler.direct((function() {
        ...
        if (((this._topPosition - this._previousTop) > 0) && !this._forward) {
          this._forward = true;
        }
    ...
    _updateViewport: function(newIndex) {
      ...
      this._updateVisibleItems(false, newIndex);
    },
    ...
    _updateVisibleItems: function(full, newIndex) {
      ...
      var criticalStart = source.indexAtPosition(top);
      var criticalEnd = source.indexAtPosition(top + viewPortHeight);

      var switchWindow = this._switchWindow;

      var recyclableItems = recycle(items, criticalStart, criticalEnd);
      var canPrerender = this._maxItemCount - (criticalEnd - criticalStart) - 1;
      var before = switchWindow;
      var after = canPrerender - switchWindow;
      ...
      if (full) {
        endIndex = Math.min(source.fullLength() - 1,
                            startIndex + this._maxItemCount - 1);
      }
      ...
      var placeItem = (function(i) {
        ...
        if (!item) {
          ...
          } else if (itemsInDOM.length < this._maxItemCount){
