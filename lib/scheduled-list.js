(function(exports) {
  ...
  list.prototype = {
    ...
    updateViewportGeometry: function() {
      ...
      var timeSinceLast = Date.now() - this._previousDate;
      this._previousDate = Date.now();
      ...
      var speed = Math.abs(distanceSinceLast / timeSinceLast);
      var fastScrolling = this._previousFast;
      if (!fastScrolling && speed > 20) {
        fastScrolling = true;
      }
      if (fastScrolling && speed < 10) {
        fastScrolling = false;
      }
    ...
    render: function(reload, changedIndex) {
      ...
      var renderItem = function(i) {
        ...
        placeItem(item, i, geo, source, reload);
      };
  ...
  function placeItem(item, index, geometry, source, reload) {
